(function () {
  ...
  if (!session || session.role !== "volunteer") location.href = "login.html";
  ...
  if (myApps) {
    const rows = db.applications.filter((item) => item.userEmail === session.email);
  ...
  if (myHelp) {
    const rows = db.helpRequests.filter((item) => item.userEmail === session.email);
    myHelp.innerHTML = rows.length ? rows.map((item) => `<tr><td>${item.requestType}</td><td>${item.community}</td><td>${item.status}</td></tr>`).join("") : `<tr><td colspan="3">No help requests yet.</td></tr>`;
  }
})();
